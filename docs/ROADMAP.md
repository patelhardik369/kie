...
the riskiest assumption — that one generated form can serve 86 heterogeneous models — is tested in
...
`ModelDefinition` types, then all 86 definitions transcribed from
...
**Exit:** 19 + 20 + 20 + 14 + 8 + 5 = 86 definitions compile. A test asserts every slug is unique and every
...
**Exit:** all 86 models render a complete, usable form with **zero model-specific branches in
...
| `/verify-catalog` | Zero drift. All 86 registry `docUrl`s match `llms.txt` exactly; 19 Kling / 20 ByteDance / 20 Wan / 14 Google / 8 OpenAI / 5 Enhance |
