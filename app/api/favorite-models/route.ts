...
/**
 * Pinned models — the shortlist above the 82.
 *
 ...
 * That is deliberate: it keeps the 82-model registry — every parameter of every
 * model — out of the browser bundle, so a nav popover costs a fetch rather than
 * a few hundred kilobytes of JavaScript.
 */

export async function GET() {
  return NextResponse.json({ favorites: await listPins() })
}
...
export async function POST(request: Request) {
  ...
  return NextResponse.json({ favorites: await addPin(slug) })
}
...
export async function DELETE(request: Request) {
  ...
  return NextResponse.json({ favorites: await removePin(slug) })
}
...
export async function PATCH(request: Request) {
  ...
  return NextResponse.json({ favorites: await reorderPins(body.order as string[]) })
}
